...
import requests
from config import (OLLAMA_URL, MODEL_NAME, TOP_K, MAX_CONTEXT_CHARS, MAX_TOKENS, TEMPERATURE, MAX_HISTORY, SEMANTIC_THRESHOLD, RESPONSE_CACHE_SIZE, CONFIDENCE_THRESHOLD, SESSION_EXPIRY, SUMMARY_THRESHOLD, CHROMA_PATH)
...
class RAGEngine:
    ...
    def _init_chroma(self):
        try:
            ...
            self._collection = client.get_or_create_collection(name = "robot_knowledge", metadata = {"hnsw:space": "cosine"})
    ...
    def ingest(self, text: str, chunk_size: int = 400) -> int:
        ...
        if self._use_chroma and self._embed_model:
            embeddings = self._embed_model.encode(new_chunks, convert_to_numpy = True, normalize_embeddings = True).tolist()
            start_id = self._collection.count()
            self._collection.add(documents = new_chunks, embeddings = embeddings, ids = [f"chunk_{start_id + i}" for i in range(len(new_chunks))])
        else:
            self._build_tfidf()
    ...
    def retrieve(self, query: str) -> tuple:
        ...
        if self._use_chroma and self._collection and self._embed_model:
            try:
                q_emb = self._embed_model.encode([query], convert_to_numpy = True, normalize_embeddings = True).tolist()
        ...
        scored = [(float(tfidf[i]) + 0.3 * self._keyword_score(q_tokens, c), c) for i, c in enumerate(self.chunks)]
        scored.sort(reverse = True)
...
class SemanticMatcher:
    ...
    def match(self, user_input: str) -> Optional[str]:
        ...
        if self.model and self.embeddings is not None:
            q_vec = self.model.encode([cleaned], convert_to_numpy = True, normalize_embeddings = True)
...
def build_system_prompt(prompts: dict) -> str:
    ...
    base = "You are Avocado, the pit assistant for FRC Team 10600 Two Steps Ahead. Answer in exactly 1-2 sentences. Neutral tone. No enthusiasm, no filler, no extra context. Use only the facts in the provided context and team info. Do not mention yourself or the team name unless directly asked. Do not repeat the question back. If the answer is not available say exactly: \"Great question. I don't have that information. Ask one of our team members at the pit.\""
    knowledge = f"\n\n=== TEAM INFORMATION ===\nTeam Name: {info.get('team_name','N/A')}\nTeam Number: {info.get('team_number','N/A')}\nLocation: {info.get('location','N/A')}\nMembers: {info.get('members','N/A')}\nSubteams: {info.get('subteams','N/A')}\nAwards: {info.get('awards','N/A')}\nOutreach: {info.get('outreach','N/A')}\nEmail: {info.get('email','N/A')}\nInstagram: {info.get('instagram','N/A')}\nWebsite: {info.get('website','N/A')}\n\n=== ROBOT INFORMATION ===\nRobot Name: {robot.get('robot_name','N/A')} ({robot.get('season','N/A')})\nDrivetrain: {robot.get('drivetrain','N/A')}\nTop Speed: {robot.get('top_speed','N/A')}\nBattery: {robot.get('battery','N/A')}\nScoring: {robot.get('scoring','N/A')}\nIntake: {robot.get('intake','N/A')}\nShooter: {robot.get('shooter','N/A')}\nLanguage: {robot.get('language','N/A')}\nAutonomous: {robot.get('autonomous','N/A')}\nData Logging: {robot.get('data_logging','N/A')}\n\n=== GAME INFORMATION ===\nGame: {game.get('game_name','N/A')}\nObjective: {game.get('objective','N/A')}\nGame Pieces: {game.get('game_pieces','N/A')}\nAutonomous: {game.get('autonomous','N/A')}\nTeleop: {game.get('teleop','N/A')}\nEndgame: {game.get('endgame','N/A')}\n"
    return base + knowledge

async def stream_ollama(question: str, context_chunks: list, session_id: str, memory: MemoryStore, system_prompt: str):
    import httpx
    context_text = "\n\n".join(context_chunks)
    ...
    try:
        async with httpx.AsyncClient(timeout = httpx.Timeout(10, read = 180)) as client:
            async with client.stream("POST", OLLAMA_URL, json = {"model": MODEL_NAME, "messages": messages, "options": {"temperature": TEMPERATURE, "num_predict": MAX_TOKENS}, "stream": True}) as resp:
