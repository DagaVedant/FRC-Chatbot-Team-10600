...
def get_analytics():
    try:
        ...
        top_q = conn.execute("SELECT message, COUNT(*) c FROM questions GROUP BY lower(message) ORDER BY c DESC LIMIT 10").fetchall()
        unanswered_q = conn.execute("SELECT message, ts FROM questions WHERE answered=0 ORDER BY ts DESC LIMIT 10").fetchall()
        conn.close()
        return {
...
def startup():
    ...
    print("Two Steps Ahead Chatbot Initialization")
    init_db()
...
async def chat(req: Request):
    ...
    if not ollama_alive():
        ...
        if instant:
            ...
            return {"reply":instant,"source":"dictionary","chunks_used":0,"session_id":session_id,"think_seconds":round(elapsed,1)}
    ...
    if confidence < CONFIDENCE_THRESHOLD and not chunks:
        ...
        return {"reply":fallback,"source":"fallback","chunks_used":0,"session_id":session_id,"think_seconds":0.0}

    cached = cache.get(msg, chunks)
    if cached:
        log_question(msg,"ai",True,0,cached=True)
        return {"reply":cached,"source":"ai","chunks_used":len(chunks),"session_id":session_id,"think_seconds":0.1,"cached":True}
